...
@section('content')
    <div class="main">
        ...
        <a href="{{ route('resident.jobs.add') }}" class="btn btn-primary btn-sm btn-outline-dark">Submit New</a>
        <div class="clearfix"></div>
        <p class="pull-left">
            @if(isset($closed) && $closed)
                <a class="maintjobs" href="{{ route("resident.jobs") }}">
        ...
        <table class="table table table-striped table-bordered">
            ...
            @forelse($jobs as $job)
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{{ route('resident.jobs.view', $job) }}">
            ...
            @empty
                <tr class="text-center">
                    @if (isset($closed) && $closed)
                        <td colspan="6">You currently no closed maintenance requests</td>
                    @else
                        <td colspan="6">You currently no open maintenance requests</td>
                    @endif
                </tr>
            @endforelse
            </tbody>
